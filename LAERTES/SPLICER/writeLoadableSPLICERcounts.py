...
DATAFILE = "test-query-of-counts-03172016.csv" # NOTE: this data comes from CSV export of the query shown below
EVTYPE = "SPL_SPLICER"
...
for elt in l:
    ...
    try:
        (cnt,drug,hoi,section) = [x.strip() for x in elt.split(",")]
    except ValueError:
        break
    q = TEMPLATE.replace("@IMEDS_DRUG@",drug).replace("@IMEDS_HOI@",hoi).replace("@SECTION@",section)
