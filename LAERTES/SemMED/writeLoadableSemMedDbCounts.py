...
#SELECT ?an ?drug ?hoi ?modality ?predicateLab ?studyType ?exact ?prefix ?postfix
SELECT count(distinct ?an) ?agents ?drug ?effect ?hoi ?modality ?predicateLab ?studyType ?exact ?prefix ?postfix
WHERE {
  ...
  ?sel oa:exact ?exact.
  OPTIONAL {
     ?sel oa:prefix ?prefix.
  }
  OPTIONAL {
     ?sel oa:postfix ?postfix.
  }

  ?body poc:modality ?modality;
        poc:semanticNetworkPredicate ?predicate.
  ?predicate rdfs:label ?predicateLab.

  {?body ohdsi:ImedsDrug ?drug.} 
  UNION 
  {
    ?body ohdsi:adeAgents ?agents.
  ...
  UNION 
  {
    ?body ohdsi:adeEffects ?effects.
    ?effects ohdsi:ImedsHoi ?hoi.
  }
}
